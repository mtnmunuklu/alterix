...
type valueComparator func(field interface{}, matcherValue interface{}) string

// baseComparator is the default comparison method used by Sigma rules, which performs a case-insensitive string comparison
func baseComparator(field interface{}, matcherValue interface{}) string {
	switch {
	case matcherValue == "null":
		...
		return fmt.Sprintf("%v = ''", coerceString(field))
	default:
		// Perform a case-insensitive string comparison
		return fmt.Sprintf("%v = '%v'", strings.ToLower(coerceString(field)), coerceString(matcherValue))
	}
}
...
var modifiers = map[string]valueModifier{
	...
	"contains": func(_ valueComparator) valueComparator {
		return func(field interface{}, matcherValue interface{}) string {
			return fmt.Sprintf("%v like '%%%v%%'", strings.ToLower(coerceString(field)), coerceString(matcherValue))
		}
	},
	...
	"re": func(_ valueComparator) valueComparator {
		return func(field interface{}, matcherValue interface{}) string {
			return fmt.Sprintf("%v re '%v'", strings.ToLower(coerceString(field)), coerceString(matcherValue))
		}
	},
	...
	"cidr": func(_ valueComparator) valueComparator {
		return func(field interface{}, matcherValue interface{}) string {
			return fmt.Sprintf("%v cidr '%v'", strings.ToLower(coerceString(field)), coerceString(matcherValue))
		}
	},
